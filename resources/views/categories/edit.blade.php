            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('categories.update', $category->id) }}">
                    ...
                    <div class="p-6 max-w-lg mx-auto">
                        ...
                        <div class="mt-6">
                            <label class="block font-medium text-sm text-gray-700 dark:text-gray-300">Pemasok yang Menyediakan Kategori Ini</label>
                            <div class="mt-2 grid grid-cols-2 sm:grid-cols-3 gap-4 p-4 border rounded-md">
                                @foreach($suppliers as $supplier)
                                    <label class="flex items-center">
                                        ...
                                        <span class="ms-2 text-sm text-gray-600 dark:text-gray-300">{{ $supplier->name }}</span>
                                    </label>
                                @endforeach
                            </div>
                        </div>
