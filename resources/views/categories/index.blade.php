                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    {{-- Kolom untuk Pemasok --}}
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Disediakan Oleh</th>
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex items-center justify-center space-x-2">
                                                <a href="{{ route('categories.show', $category->id) }}" class="p-2 text-gray-400 hover:text-blue-600 rounded-md hover:bg-blue-100" title="Detail"><i class="fa-solid fa-eye"></i></a>
                                                @can('manage-content')
                                                    <a href="{{ route('categories.edit', $category->id) }}" class="p-2 text-gray-400 hover:text-yellow-600 rounded-md hover:bg-yellow-100" title="Edit"><i class="fa-solid fa-pencil"></i></a>
                                                    <form action="{{ route('categories.destroy', $category->id) }}" method="POST" onsubmit="return confirm('Yakin hapus?')" class="inline-block">
                                                        @csrf @method('DELETE')
                                                        <button type="submit" class="p-2 text-gray-400 hover:text-red-600 rounded-md hover:bg-red-100" title="Hapus"><i class="fa-solid fa-trash"></i></button>
                                                    </form>
                                                @endcan
                                            </div>
                                        </td>
