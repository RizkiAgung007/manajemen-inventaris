<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div x-data="{ tab: 'info' }" class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">

                <div class="border-b border-gray-200 dark:border-gray-700">
                    <nav class="-mb-px flex" aria-label="Tabs">
                        <button @click="tab = 'info'"
                                :class="{ 'border-indigo-500 text-indigo-600': tab === 'info', 'border-transparent text-gray-500 dark:text-gray-300 hover:text-gray-700 hover:border-gray-300': tab !== 'info' }"
                                class="w-1/2 py-4 px-1 text-center border-b-2 font-medium text-sm">
                            Informasi Pengguna
                        </button>
                        <button @click="tab = 'log'"
                                :class="{ 'border-indigo-500 text-indigo-600': tab === 'log', 'border-transparent text-gray-500 dark:text-gray-300 hover:text-gray-700 hover:border-gray-300': tab !== 'log' }"
                                class="w-1/2 py-4 px-1 text-center border-b-2 font-medium text-sm">
                            Log Aktivitas
                        </button>
                    </nav>
                ...
                <div x-show="tab === 'info'" class="p-6">
                    <div class="flex justify-between items-start mb-4">
                        <div>
                            <h3 class="text-lg font-medium text-gray-900 dark:text-gray-300">Data Diri Pengguna</h3>
                        </div>
                    ...
                    <dl class="border-t border-gray-200 pt-4 grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                        <div>
                            <dt class="font-medium text-gray-500 dark:text-gray-100">Nama</dt>
                            <dd class="mt-1 text-gray-900 dark:text-gray-300 font-semibold">{{ $user->name }}</dd>
                        </div>
                        <div>
                            <dt class="font-medium text-gray-500 dark:text-gray-100">Email</dt>
                            <dd class="mt-1 text-gray-900 dark:text-gray-300">{{ $user->email }}</dd>
                        </div>
                        <div>
                            <dt class="font-medium text-gray-500 dark:text-gray-100">Role</dt>
                            <dd class="mt-1"><x-role-badge :role="$user->role" /></dd>
                        </div>
                        <div>
                            <dt class="font-medium text-gray-500 dark:text-gray-100">Tanggal Bergabung</dt>
                            <dd class="mt-1 text-gray-900 dark:text-gray-300">{{ $user->created_at->format('d F Y, H:i') }}</dd>
                        </div>
                    </dl>
                </div>
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse ($activities as $log)
                                    <tr class="dark:hover:bg-gray-700">
                                        <td class="px-6 py-4 dark:text-gray-300">{{ $log->activity }}</td>
                                        <!-- <td class="px-6 py-4 text-sm text-gray-500">{{ $log->ip_address }}</td> -->
                                        <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-300" title="{{ $log->created_at->format('d F Y, H:i:s') }}">
